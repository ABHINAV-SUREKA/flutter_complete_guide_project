  ...
  Widget build (BuildContext context) { // gets rebuild when setState() forces the affected ServicesList widget (through the external ServicesList() constructor call) to re-render (in main.dart)
    return new Column(
      children: [
        ...
        ...(serviceListMap[listIndex]["institutions"] as List<String>).map((institution) { // using 'as List<String>' to convert Object type (as we have mentioned value as Object in Map) to List<String>  //.map() generates a new list based on old list
          return new Institution(callbackFunction, institution); // returns Institution (Button) widget
